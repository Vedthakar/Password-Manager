...
const PORT = 3001; // Assuming you're using this port now
    ...
    req.on('end', async () => {
      ...
      try {
        ...
        if (parsedData && parsedData.hostname !== undefined && parsedData.isLoginPage !== undefined) {
          ...
          // Read hostname from pages.json and add it to parsedData
          ...
          // Make the outgoing POST request to the master password server
          try {
            ...
            // --- MODIFIED: Parse Flask's response to get the 'message', 'status', AND 'password' ---
            let flaskMessage = "Unknown response from Flask server.";
            let flaskStatus = "error"; // Default status
            let foundPassword = null; // Initialize to null
            ...
            try {
                ...
                foundPassword = flaskResponseBody.password || null; // Get the password from Flask
            ...
            res.end(JSON.stringify({
              ...
              status: flaskStatus,
              password: foundPassword // Pass the found password to the extension
            }));
          } catch (postError) {
            ...
            res.end(JSON.stringify({
              ...
              password: null // No password on error
            }));
          }
