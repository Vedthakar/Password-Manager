...
masterPasswordCheck = b'a1DOnVGLGvq08ZI2MigPQzjBwRxGhPQ+GsPaegpNSjU=' # Generated from the master password "vedthakar"
...
def hostnames_match(requested_hostname, stored_hostname):
    requested = normalize_hostname(requested_hostname)
    stored = normalize_hostname(stored_hostname)

    if not requested or not stored:
        return False

    return requested == stored or requested in stored or stored in requested
...
def receive_master_pass_json():
    ...
    if request.is_json:
        ...
        search_candidates = [
            received_data.get('hostname'),
            received_data.get('loginPageHostname')
        ]
        ...
        try:
            ...
            if encrypted_check_with_received_password == masterPasswordCheck:
                ...
                if search_candidates:
                    found_password = None
                    current_passwords = saver.read()

                    for entry in current_passwords:
                        try:
                            # entry[0] is the encrypted application name
                            decrypted_app_name = encrypter.decrypt(entry[0])
                            if any(hostnames_match(candidate, decrypted_app_name) for candidate in search_candidates):
                                found_password = encrypter.decrypt(entry[1]) # entry[1] is the encrypted password
                                print(f"[Search] Found password for {search_candidates[0]}: {found_password}")
                                # Return the found password directly
                                return jsonify({
                                    "status": "success",
                                    "message": f"Password found for {search_candidates[0]}",
                    ...
                    print(f"[Search] No password found for application: {search_candidates[0]}")
                    return jsonify({
                        "status": "success", # Authentication was still successful
                        "message": f"No password found for application: {search_candidates[0]}",
